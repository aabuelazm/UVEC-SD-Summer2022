...
#pragma config(Motor, port2, mArm, tmotorVex393_MC29, PIDControl, reversed,    \
               encoderPort, I2C_1)
#pragma config(Motor, port3, mArm1, tmotorVex393_MC29, PIDControl,             \
               encoderPort, I2C_2)
#pragma config(Motor, port4, mClaw, tmotorVex393_MC29, openLoop, encoderPort,  \
               I2C_3)
...
enum SystemPhase { idle, moveFore, pickup, moveBack, egg };
...
task main() {
  ...
  while (!done) {
    ...
    case egg:
      moveMotorTarget(mArm, ARM_DROP, ARM_SPEED, true);
